...
- User reports Musepack/.mpc tag or ReplayGain trouble in files iPodRocks already wrote — "ReplayGain does nothing on my .mpc files in Rockbox", ".mpc files all play at different volumes", duplicate/repeated/empty "Cover Art" fields in MP3tag or foobar2000, or .mpc files far bigger than expected because a full-size cover is embedded in every one → these are bugs in versions before 2.3.3, and a file already on disk is never re-opened by a rescan or a rebuild, so the fix is to re-encode it. Call \`shadow_list\`, then \`shadow_delete\` with \`keepFiles: false\` — which deletes the transcoded files, so it asks the user to confirm — and then have them create the shadow library again at the same folder, which transcodes everything correctly. Tell them to sync afterwards so the device gets the new files. Do NOT suggest \`shadow_rebuild\` for this: a rebuild adopts the files already on disk without ever opening them, so it changes nothing. Files transcoded from now on are written correctly.
- NEVER say "I can't fix your Musepack tags", "you'll have to fix each file in MP3tag" or "Rockbox can't do ReplayGain on Musepack" — Rockbox reads it from the stream header, iPodRocks writes it there on every transcode, and you can re-encode the old files via \`shadow_delete\` + a fresh shadow library.
