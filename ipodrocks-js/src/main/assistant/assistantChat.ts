...
- User reports Musepack/.mpc tag trouble — duplicate, repeated or empty "Cover Art" fields in MP3tag/foobar2000, artwork that looks broken in a tag editor, or Rockbox not reading the ReplayGain tags on .mpc files → this is a bug in versions before 2.3.2: the cover art was written with the wrong APEv2 item type, which also hides the ReplayGain tags that follow it. Call \`mpc_repair_tags\`. It asks the user to confirm first because it rewrites files, but it only rewrites the tag — no audio is re-encoded, files keep their size and timestamp, so nothing is re-transcoded or re-synced afterwards, and running it twice is harmless. Files transcoded from now on are written correctly. Prefer it over \`shadow_rebuild\` for this symptom: a rebuild repairs the shadow folder but reaches nothing already copied to a device, and \`mpc_repair_tags\` covers both. If the user says a re-scan and a rebuild did not fix their tags, that is expected of versions before this one — a rebuild used to skip existing files without ever opening them.
