...
describe("buildTagnaviConfig", () => {
  ...
  it("includes required header lines", () => {
    ...
    expect(config).toContain('%menu_start "custom" "iPodRocks Smart"');
  });
  ...
  it("includes firmware default formats and menus (overrides tagnavi.config)", () => {
    ...
    expect(config).toContain('"Custom menu"  ==> "custom"');
    expect(config).toContain('%root_menu "main"');
  });

  it("empty input → smart menu present but with no entries", () => {
    const config = buildTagnaviConfig([], { now: fixedNow });
    const smartMenuIdx = config.indexOf('%menu_start "custom"');
    const afterSmart = config.slice(smartMenuIdx);
    const nextMenuIdx = afterSmart.indexOf("%byfirstletter");
    const smartBlock = afterSmart.slice(0, nextMenuIdx);
    expect(smartBlock).not.toMatch(/^".*" -> title/m);
  });
