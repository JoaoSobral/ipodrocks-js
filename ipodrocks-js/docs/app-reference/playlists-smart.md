...
- **Reboot to reload the config.** Rockbox caches `tagnavi_user.config` in memory and only re-reads it on boot. After a sync that changes smart playlists, reboot the player so Rockbox picks up the new entries (on an iPod, hold **Menu+Select** for ~5 seconds; other players have their own reset combination). Without a reboot, the menu still shows the previous version of the file regardless of what was just synced.
