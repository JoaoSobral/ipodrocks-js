...
- **Rebuild** — The **⟳** button on a shadow library row. It does not re-encode what is already there: files that match the library's codec configuration are adopted as they stand, and only missing or mismatched tracks are converted. It does, however, read the tag block of every Musepack file in the folder and repair the malformed cover art written by versions before 2.3.2 — the audio is untouched and the files keep their size and timestamp, so nothing is re-transcoded and your next sync copies nothing extra. Copies already on a device are a separate matter: use [Settings → Maintenance → Repair Musepack tags](./settings.md), which covers connected devices as well.
