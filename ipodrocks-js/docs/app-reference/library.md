...
- **Rebuild** — The **⟳** button on a shadow library row. It does not re-encode what is already there: files that match the library's codec configuration are adopted as they stand, and only missing or mismatched tracks are converted. It does, however, read the tag block of every Musepack file in the folder and repair what earlier versions wrote there — the malformed cover art from before 2.3.2, and, from 2.3.3, any embedded artwork at all plus any missing ReplayGain. The audio is untouched and the files keep their timestamp, so nothing is re-transcoded; they do get smaller when artwork comes out, so your next sync re-copies those. Copies already on a device are a separate matter: use [Settings → Maintenance → Repair Musepack tags](./settings.md), which covers connected devices as well.
