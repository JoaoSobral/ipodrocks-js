...
6. **Rockbox smart playlists (tagnavi)** — When enabled, smart playlists sync as live `.rockbox/tagnavi_custom.config` entries instead of static `.m3u` files. See [Smart Playlists → Rockbox dynamic mode](./playlists-smart.md#rockbox-dynamic-mode-per-device-opt-in).
