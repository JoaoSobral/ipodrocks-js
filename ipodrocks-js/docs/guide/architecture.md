...
| **RuntimeIngest** | Reads Rockbox's `database_idx.tcd` off the device into `device_runtime_stats`, `runtime_play_deltas` and `playback_stats`, and writes rating changes back into the same file. |
