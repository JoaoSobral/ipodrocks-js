...
- **Eject a device without leaving iPodRocks.** Devices → **Eject**, on each device card. It unmounts the volume so you can safely unplug the player, instead of switching to Finder to do it. **macOS and Linux only** — the button is hidden on Windows, where there is no dependable command-line eject; use Explorer's Safely Remove Hardware there. Nothing is deleted and nothing is written to the device. It refuses, and says why, if a sync is running, if the device is in dev mode, or if the mount path is not actually a mounted volume. Rocksy can eject too, with `device_eject`; it asks you to confirm first.
