...
- **Ratings now reach tracks your player has never rated.** (#138) A newly synced album arrived on the device with no ratings at all, however many times you synced afterwards. iPodRocks would only send a rating for a track the player had *already reported a rating for* — which a track it has just been given never has — so anything new was invisible to the step that writes them, permanently. It was worst right after reinitialising the player's database, because that path skips reading ratings in, so nothing could break the deadlock. Every rated track on the device is now considered. Two related cases in the same step: a write that failed because the database was missing or Rockbox was busy updating it was recorded as if it had succeeded, so it was never retried; and one unreadable record threw away the bookkeeping for every rating written before it. Both are fixed, and a sync that cannot write a rating now says so and retries next time.

- **The sync log explains ratings it could not write yet.** On the sync that *copies* an album, Rockbox does not yet know those files exist, so there is nowhere to put a rating — unavoidable, and previously silent, which made it look like the ratings had been dropped. The log now tells you to run **Database → Update now** on the player and sync again, and iPodRocks re-sends them by itself. Rocksy knows to explain this too.

- **Re-rating a track on your player no longer raises a conflict with yourself.** (#138) Any track you had ever rated inside iPodRocks was treated as "changed on both sides" forever after, so changing its rating on the player asked you to resolve a disagreement that did not exist — or, when the two values were a half-star apart, silently kept the higher one and discarded what you set on the device. iPodRocks now remembers which version of a rating it sent, so a change made only on the player is simply accepted — including after the player has reported that rating back, which is what every normal sync does and which kept the bug alive one round longer. Genuine disagreements are still raised. Existing libraries are repaired when the app next opens.

- **ReplayGain now actually works on Musepack files in Rockbox.** (#137) Musepack is the one format that keeps its ReplayGain in the file's stream header rather than in its tag, and the header is the only place Rockbox looks. iPodRocks wrote the values into the tag — where every desktop player reads them, and your iPod does not — while the header stayed as the encoder left it: zeroed. So nothing was ever applied on the player. Not the album gain that prompted this, and not the track gain either. New transcodes write all four values into the header, and once the header carries them the tag copy is removed, so there is one source of truth. To fix the `.mpc` files you already have, **delete the shadow library — choosing to delete its files too — and create it again**: that re-encodes everything correctly, and because the new files differ in size and timestamp your next sync carries them out to the player as well. Older SV7 (`MP+`) Musepack files have no such header; they are left alone and keep using their tag.
...
- **Album artwork is no longer embedded into Musepack files.** (#130) Every track carried a full-resolution copy of the album cover — 1500×1500 in the reported case — which is pure weight on a player: Rockbox reads the `cover.jpg` iPodRocks already writes beside the audio, resized to 300px. Nothing is embedded any more. To shrink the files you already have, delete the shadow library with its files and create it again — the re-encoded files are noticeably smaller, and your next sync re-copies them.

### Removed

- **Settings → Maintenance → Repair Musepack tags is gone**, along with the tag-repair pass that ran inside a shadow library rebuild. Rewriting files in place had to reproduce every decision the transcoder makes, in a second implementation that could only ever drift from it. Re-encoding is both simpler and more thorough: **delete the shadow library, choosing to delete its files too, then create it again.** That rewrites every file from the source — correct tags, ReplayGain in the stream header, no bloated embedded covers — and the new files differ enough that your next sync copies them to the player, which the in-place repair could never manage on its own. Files transcoded by this version were always written correctly; this only concerns ones an older version left behind. Ask Rocksy and it will walk you through it.

## [2.3.2] — 2026-09

### Features

- **Eject a device without leaving iPodRocks.** The **⏏** button on each device card in Devices. It asks you to confirm, then unmounts the volume so you can safely unplug the player, instead of switching to Finder to do it. Nothing is deleted and nothing is written to the device. The button is greyed out when it cannot be used — on Windows, where there is no dependable command-line eject (use Explorer's Safely Remove Hardware there), and whenever the device is not connected — and hovering it tells you which. It also refuses, and says why, if a sync is running, if the device is in dev mode, or if the mount path is not actually a mounted volume. Rocksy can eject too, with `device_eject`; it asks you to confirm first.
...
- **"Don't remind me" about Musepack now actually sticks.** The Devices panel decided whether to show the "mpcenc is not installed" reminder before it had finished reading that preference, so on any launch where that one lookup answered last, the reminder appeared anyway — and having appeared once, it stayed the decision for the rest of the session.

- **Rebuilding a shadow library now repairs the Musepack tags it used to skip.** (#130) The advice for the tag bug below was "re-scan, then rebuild the shadow library", and it did nothing at all: a rebuild adopts files that are already correctly encoded, and it decided that from their size and timestamp without ever opening them — so it never saw the tag, let alone rewrote it. Clearing the scan cache made no difference for the same reason. A rebuild now reads the tag block of every Musepack file in the shadow folder and repairs the ones that need it, before it converts anything. The audio is untouched and the files keep their size and timestamp, so nothing is re-transcoded and your next sync copies nothing extra. Copies already sitting on a device are not reached by this — the build log says so, and Settings → Maintenance → Repair Musepack tags covers those.
...
- **Statistics now come straight from Rockbox.** iPodRocks reads Rockbox's own Gather Runtime Data (play counts, listening time, ratings) instead of the old text-based playback log. This is one setting instead of two, and it kills the old bug where an unset/dead iPod clock stamped every play as "year 2000." Enable Settings → Playback Settings → Gather Runtime Data on the device.

- **Ratings now sync both ways and actually land.** Rate on either side and it reaches the other. The old method wrote a file Rockbox couldn't read; ratings are now written directly into the field Rockbox uses, so they survive a database rebuild. (Heads-up: with Load to RAM on, a new rating only shows on the device screen after Rockbox restarts.)

- **Genius playlists rebuilt on the new data**, with some bug fixes along the way: Top Artist/Album now weight by play count instead of counting each track once, and Hidden Gems again means "zero plays." Late Night Mood is gone (needed a clock the data doesn't have) and replaced by Forgotten Favorites; Skip List is now Never Finished.

- **Clearer Listening Stats** — All Time shows full totals; This Year/This Month count plays seen since 2.3.0-beta, dated by your computer's clock.

- **Device Status card in the Sync panel shows what's already on the device** (songs, podcasts, orphans, space used) before you sync.

- **Devices identified by USB hardware**, not just mount path — so two iPods that both mount at /Volumes/IPOD no longer get confused. Recognized models are named automatically.

- **Albums grouped by album artist** — compilations are one album again instead of one row per contributing artist.

- **Prune orphan files from a shadow library** via the new ⚙ button, cleaning out stale/duplicate transcodes.
...
- **Shadow libraries** no longer pile up copies of renamed/deleted albums.
- **A USB-bound device** can no longer sync to the wrong volume.
- **Album artwork** settings now save on the first device-add.
- **Devices with no USB serial number** now say so instead of implying uniqueness.
- **USB enumeration** failing now falls back to mount-path matching instead of marking every device offline.
  
### Changed
Play counts and listening time restart from Rockbox's own counters. The old log counted plays over half a second; runtime data counts only plays past 15 seconds, so the two can't be combined. Old data is left on disk but unused.
Device option "Do not read playback.log data" is renamed to "Do not import play history from this device."
